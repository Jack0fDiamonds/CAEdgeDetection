...
//  //toGreyscale(input, greyScale);
//OTCAStep(output);
//  binaryEdgeDetection2(output);
...
//  //PImage img1 = createImage(input.width, input.height, RGB);
//  //PImage img2 = createImage(input.width, input.height, RGB);
//  //PImage img3 = createImage(input.width, input.height, RGB);
//  //toBinary(input, img1, 111);
//  //toBinary(input, img2, 131);
//  //toBinary(input, img3, 151);
//  //image(img1, 0, 0);
//  //image(img2, w, 0);
//  //image(img3, w*2, 0);
//}

void TCAStep(PImage in) {
  PImage buffer = in.copy();
  in.loadPixels();

  for (int x = 1; x < w-1; x++) {
    for (int y = 1; y < h-1; y++ ) {
      int loc = x + y*in.width;
      if (in.pixels[loc] == color(255)) {
        int sum = binaryMooreNeighborSumIn(buffer, x, y);
        if (sum > 2 && sum < 6)
          in.pixels[loc] = color(255);
        else
          in.pixels[loc] = color(0);
      }
    }
  }
...
void OTCAStep(PImage in) {
  ...
  for (int x = 1; x < w-1; x++) {
    for (int y = 1; y < h-1; y++ ) {
      int loc = x + y*in.width;
      int sum = binaryMooreNeighborSumIn(buffer, x, y);
      if (sum == 4 || (sum == 3 && brightness(buffer.pixels[loc]) == 255))
        in.pixels[loc] = color(255);
      else
        in.pixels[loc] = color(0);
    }
...
void binaryEdgeDetection(PImage in) {
  PImage buffer = in.copy();
  in.loadPixels();
  ...
  for (int x = 1; x < w-1; x++) {
    for (int y = 1; y < h-1; y++ ) {
      ...
      if (brightness(buffer.pixels[loc]) == 255) {
        if (binaryMooreNeighborSumEx(buffer, x, y) == 8)
          in.pixels[loc] = color(0);
      }
    }
  }
...
void binaryEdgeDetection2(PImage in) {
  PImage buffer = in.copy();
  in.loadPixels();
  ...
  //  treshold++;
  //  println(treshold);
  //  toBinary(input, binary, treshold);
  //  output = binary.copy();
  //}
}

void updateImage() {
  if (tresholdIncreasing) {
    treshold++;
    if (treshold == 255)
      tresholdIncreasing = false;
  } else {
    treshold--;
    if (treshold == 0)
      tresholdIncreasing = true;
  }
  ...
  for (int x = 1; x < w; x++) {
    for (int y = 0; y < h; y++ ) {
      int loc = x + y*input.width;
      if (brightness(input.pixels[loc]) < treshold) {
        binary.pixels[loc] = color(0);
      } else {
        binary.pixels[loc] = color(255);
      }
    }
  }
  binary.updatePixels();
